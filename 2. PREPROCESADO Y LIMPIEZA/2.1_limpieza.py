"""
MÓDULO DE LIMPIEZA Y ESTANDARIZACIÓN DE DATOS RAW
--------------------------------------------------
Este script procesa los datos brutos extraídos en la fase de adquisición.
Aplica normalización de texto (Regex), conversión estricta de tipos de datos,
y eliminación de outliers mediante el Rango Intercuartílico (IQR). Destaca
el uso de Machine Learning (IterativeImputer) para la imputación inteligente
de valores nulos basada en correlaciones multivariantes.
"""
...
if __name__ == "__main__":
    ruta_datos = "../DATOS/tecnocasa_datos_sin_limpiar.parquet"
    urls_localizacion = [
        "../DATOS/distrito_barrios.parquet", #son csv
        "../DATOS/municipios.parquet"
    ]
    rutas_coords_csv = [
        "../DATOS/coords_distrito.csv", #si son nuevos datos hay que borrar estos archivos
        "../DATOS/coords_localizacion.csv"
    ]

    df_limpio = limpiar_datos_tecnocasa(ruta_datos, urls_localizacion)

    # Guardar resultado
    df_limpio.to_parquet("../DATOS/tecnocasa_datos_limpios.parquet", index=False)
